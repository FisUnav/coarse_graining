...
namespace coarse_graining{
    ...
    grid2D::grid2D(double _x, double _y, double gridRes, vector<int> _neighbors): x(_x), y(_y), areaTerm(gridRes*gridRes), momentum_check(false), velocity_check(false) {
    ...
    void grid2D::density_field(const MatrixXd &data,
        ...
        Matrix<T, neighbors.size(), Dynamic> relData = data(neighbors, all);
        density = 0;
        volFrac = 0;
    ...
    void grid2D::velocity_field(const MatrixXd &data, 
        ...
        if (!momentum_check) momentum_field(const MatrixXd &data, double (*cg_function)(double, double, double, double));
    ...
    void grid2D::kstress_field(const MatrixXd &data, 
        ...
        if (!velocity_check) velocity_field(const MatrixXd &data, double (*cg_function)(double, double, double, double));
        Matrix<T, neighbors.size(), Dynamic> relData = data(neighbors, all);
        double densityP, vxx, vxy, vyy;
        kstress(0) = 0;
        kstress(1) = 0;
        kstress(2) = 0;
        kstress(3) = 0;
